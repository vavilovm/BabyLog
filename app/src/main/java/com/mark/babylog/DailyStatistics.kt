...
internal fun dailySummary(events: List<BabyEvent>): DailySummary {
    ...
    val sleeps =
        events
            ...
            .mapNotNull { runCatching { SleepPosition.valueOf(it.detail) }.getOrNull() }
    val pumping =
        events
            .filter { it.type == EventType.PUMPING }
            .mapNotNull { it.detail.substringAfter(':', "").toIntOrNull() }
    return DailySummary(
        feedingCount = feedings.size,
        averageFeedingIntervalMs =
            intervals.takeIf { it.isNotEmpty() }?.average()?.roundToInt()?.toLong(),
...
@Composable
internal fun DailyStatisticsDialog(
    state: DailyStatisticsState,
    onDismiss: () -> Unit,
    ...
    Dialog(
        ...
        Surface(
            ...
            Column {
                Row(
                    Modifier.fillMaxWidth().padding(20.dp, 12.dp, 8.dp, 4.dp),
                ...
                if (state.loading) {
                    ...
                    Column(
                        ...
                        if (summary.eventCount == 0) EmptyStatistics()
                            ...
                            if (summary.sleepLeftCount + summary.sleepRightCount > 0)
                                SleepStatistics(summary)
...
@Composable
private fun DayPicker(day: LocalDate, today: LocalDate, onDayChange: (LocalDate) -> Unit) {
    val relative =
        when (day) {
            today -> "Сегодня"
    ...
    val formatted =
        day.format(DateTimeFormatter.ofPattern("EEEE, d MMMM", Locale("ru"))).replaceFirstChar {
    ...
    Row(
        ...
        IconButton(onClick = { onDayChange(day.minusDays(1)) }) {
        ...
        IconButton(onClick = { onDayChange(day.plusDays(1)) }, enabled = day < today) {
    ...
    Column(
        ...
        Surface(
            ...
            Box(contentAlignment = Alignment.Center) {
                Icon(Icons.Default.Insights, null, Modifier.size(36.dp))
            }
        }
        Text("За этот день записей нет", fontWeight = FontWeight.Bold, fontSize = 18.sp)
        Text(
            "Перейдите на другой день стрелками выше",
            color = MaterialTheme.colorScheme.onSurfaceVariant,
...
@Composable
private fun FeedingStatistics(summary: DailySummary) {
    StatisticSection(icon = { Icon(Icons.Default.Restaurant, null) }, title = "Кормления") {
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Metric(
                "${summary.feedingCount}",
                plural(summary.feedingCount, "кормление", "кормления", "кормлений"),
                Modifier.weight(1f),
            )
            Metric(
                summary.averageFeedingIntervalMs?.let { durationShort(it) } ?: "—",
                "в среднем каждые",
                Modifier.weight(1f),
            )
        }
        ...
        if (summary.bottleCount > 0) {
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text(
                    "${summary.bottleVolumeMl} мл · ${summary.bottleCount} ${plural(summary.bottleCount,"раз","раза","раз")}",
                    fontWeight = FontWeight.Bold,
                )
            }
        }
        if (summary.firstFeedingAt != null) {
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Column(Modifier.weight(1f)) {
                    Text("Ритм дня", fontWeight = FontWeight.SemiBold)
                    Text(
                        "Первое ${statClock(summary.firstFeedingAt)} · последнее ${statClock(summary.lastFeedingAt!!)}",
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                    )
                }
                summary.longestFeedingIntervalMs?.let {
                    Column(horizontalAlignment = Alignment.End) {
                        Text(durationShort(it), fontWeight = FontWeight.Bold)
                        Text(
                            "макс. пауза",
...
@Composable
private fun BreastBalance(summary: DailySummary) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
        Row {
            Text("Баланс груди", fontWeight = FontWeight.SemiBold, modifier = Modifier.weight(1f))
            Text(
                "L ${summary.leftFeedingCount}  ·  R ${summary.rightFeedingCount}",
                style = MaterialTheme.typography.bodySmall,
            )
        }
...
@Composable
private fun SleepStatistics(summary: DailySummary) {
    val total = summary.sleepLeftCount + summary.sleepRightCount
    val favorite =
        when {
            summary.sleepLeftCount > summary.sleepRightCount -> "Чаще на левом боку"
            summary.sleepRightCount > summary.sleepLeftCount -> "Чаще на правом боку"
            else -> "Поровну на обоих боках"
        }
    StatisticSection(
        icon = {
            SleepSideImage(
                if (summary.sleepLeftCount >= summary.sleepRightCount) SleepPosition.LEFT
                else SleepPosition.RIGHT,
                Modifier.size(34.dp),
            )
        },
        title = "Положение во сне",
    ) {
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Text(
            "$favorite · всего $total ${plural(total,"отметка","отметки","отметок")}",
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
    }
...
@Composable
private fun PumpingStatistics(summary: DailySummary) {
    StatisticSection(icon = { Icon(Icons.Default.WaterDrop, null) }, title = "Сцеживание") {
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Metric(
                "${summary.pumpingVolumeMl/summary.pumpingCount} мл",
                "в среднем",
                Modifier.weight(1f),
            )
            Metric(
                "${summary.pumpingCount}",
                plural(summary.pumpingCount, "раз", "раза", "раз"),
                Modifier.weight(1f),
            )
        }
    }
...
@Composable
private fun SideMetric(position: SleepPosition, count: Int, modifier: Modifier) {
    Surface(
        modifier,
        shape = RoundedCornerShape(14.dp),
        color = MaterialTheme.colorScheme.surface,
        ...
        Row(Modifier.padding(10.dp), verticalAlignment = Alignment.CenterVertically) {
            SleepSideImage(position, Modifier.size(42.dp))
            Spacer(Modifier.width(8.dp))
            Column {
...
private fun durationShort(ms: Long): String {
    ...
    return when {
        hours > 0 && minutes % 60 > 0 -> "$hours ч ${minutes%60} мин"
        hours > 0 -> "$hours ч"
        else -> "$minutes мин"
    }
}

private fun statClock(ms: Long) =
