...
class BabyLogRepository(private val db:BabyDatabase){
    ...
    suspend fun startSleep(position:SleepPosition,time:Long=System.currentTimeMillis())=db.withTransaction{val owner=dao.membership();val event=dao.startSleep(position,time,owner);enqueue("START",event,time)}
