...
class ReminderRepository(
    private val app: BabyLogApp,
    private val database: BabyDatabase = app.database,
    private val requestSync: () -> Unit = { app.familySync.schedule() },
    private val reschedule: suspend () -> Unit = { ReminderScheduler.rescheduleAll(app) },
    private val clearFired: (String) -> Unit = { ReminderScheduler.clearFired(app, it) },
) {
    private val dao
        get() = database.events()

    val reminders
        get() = dao.observeReminders()

    fun completions(epochDay: Long) = dao.observeReminderCompletions(epochDay)
    ...
    suspend fun ensureDefaults() {
        ...
        if (dao.reminderCount() == 0) {
            ...
            dao.putReminder(
                BabyReminder(
                    id = "default-vitamin-k",
                    title = "Дать витамин К",
    ...
    suspend fun save(value: BabyReminder) {
        ...
        val saved =
            value.copy(
                ...
                anchorEpochDay =
                    if (value.isOneTime) value.anchorEpochDay
                    else if (old != null && old.intervalDays == value.intervalDays)
                        old.anchorEpochDay
        ...
        if (owner != null) {
            queue("REMINDER_UPSERT", reminderJson(saved), now)
            requestSync()
        }
    ...
    suspend fun delete(value: BabyReminder) {
        ...
        if (owner != null) {
            queue("REMINDER_DELETE", reminderJson(deleted), now)
            requestSync()
        }
    ...
    suspend fun setEnabled(value: BabyReminder, enabled: Boolean) =
        save(value.copy(enabled = enabled))
    ...
    suspend fun complete(reminderId: String, epochDay: Long) {
        ...
        if (owner != null) {
            queue("REMINDER_COMPLETE", completionJson(value), now)
            requestSync()
        }
        ReminderScheduler.cancelNotification(app, reminderId)
        ReminderScheduler.cancel(app, reminderId)
        dao.reminder(reminderId)
            ?.takeIf { it.enabled && it.deletedAt == null }
            ?.let { ReminderScheduler.schedule(app, it, includeMissedToday = false) }
    }
    ...
    suspend fun undo(reminderId: String, epochDay: Long) {
        ...
        val value =
            (existing ?: ReminderCompletion(reminderId, epochDay, completedAt = now)).copy(
                updatedAt = now,
                deletedAt = now,
                syncState = if (owner == null) SyncState.LOCAL_ONLY else SyncState.PENDING,
            )
        dao.putReminderCompletion(value)
        if (owner != null) {
            queue("REMINDER_UNDO", completionJson(value), now)
            requestSync()
        }
        dao.reminder(reminderId)
            ?.takeIf { it.enabled && it.deletedAt == null }
            ?.let { ReminderScheduler.schedule(app, it, includeMissedToday = true) }
    }

    suspend fun attachToFamily() {
        if (dao.membership() == null) return
        var queued = false
        dao.allRemindersForSync()
            .filter { it.syncState == SyncState.LOCAL_ONLY }
            .forEach { value ->
                val pending = value.copy(syncState = SyncState.PENDING)
                dao.putReminder(pending)
                queue(
                    if (value.deletedAt == null) "REMINDER_UPSERT" else "REMINDER_DELETE",
                    reminderJson(pending),
                    pending.updatedAt,
                )
                queued = true
            }
        dao.allReminderCompletionsForSync()
            .filter { it.syncState == SyncState.LOCAL_ONLY }
            .forEach { value ->
                val pending = value.copy(syncState = SyncState.PENDING)
                dao.putReminderCompletion(pending)
                queue(
                    if (value.deletedAt == null) "REMINDER_COMPLETE" else "REMINDER_UNDO",
                    completionJson(pending),
                    pending.updatedAt,
                )
                queued = true
            }
    ...
    suspend fun applyRemoteReminders(values: List<Map<String, Any?>>) {
        values.forEach { raw ->
            ...
            if (!shouldApplyRemote(existing?.syncState, existing?.updatedAt ?: 0, remoteUpdatedAt))
                return@forEach
            val value =
                BabyReminder(
                    id = id,
                    ...
                    intervalDays =
                        ((raw["intervalDays"] as? Number)?.toInt() ?: 1).coerceAtLeast(1),
                    anchorEpochDay =
                        (raw["anchorEpochDay"] as? Number)?.toLong()
            ...
            if (value.deletedAt != null) {
                ...
                ReminderScheduler.cancelNotification(app, id)
            }
        }
        reschedule()
    }
    ...
    private suspend fun queue(command: String, payload: JSONObject, time: Long) {
        dao.put(SyncOperation(command = command, payload = payload.toString(), occurredAt = time))
    }

    private fun reminderJson(value: BabyReminder) =
        JSONObject().apply {
            put("id", value.id)
            put("title", value.title)
            put("hour", value.hour)
            put("minute", value.minute)
            put("intervalDays", value.intervalDays)
            put("anchorEpochDay", value.anchorEpochDay)
            put("enabled", value.enabled)
            put("createdAt", value.createdAt)
            put("updatedAt", value.updatedAt)
            put("deletedAt", value.deletedAt ?: JSONObject.NULL)
        }

    private fun completionJson(value: ReminderCompletion) =
        JSONObject().apply {
            put("reminderId", value.reminderId)
            put("scheduledEpochDay", value.scheduledEpochDay)
...
) = localState != SyncState.PENDING || localUpdatedAt <= remoteUpdatedAt

internal fun BabyReminder.occursOn(date: LocalDate): Boolean =
    if (isOneTime) date.toEpochDay() == anchorEpochDay
    else
        date.toEpochDay() >= anchorEpochDay &&
            Math.floorMod(date.toEpochDay() - anchorEpochDay, intervalDays.toLong()) == 0L

val BabyReminder.isOneTime: Boolean
    get() = intervalDays == ONE_TIME_INTERVAL_DAYS
...
object ReminderScheduler {
    ...
    suspend fun schedule(context: Context, reminder: BabyReminder, includeMissedToday: Boolean) {
        ...
        val next =
            nextOccurrence(reminder, now, includeMissedToday) { day ->
                app.database.events().reminderCompletion(reminder.id, day.toEpochDay()) != null
            } ?: return
        val alarm = context.getSystemService(Context.ALARM_SERVICE) as AlarmManager
        alarm.setAndAllowWhileIdle(
            AlarmManager.RTC_WAKEUP,
            next.toInstant().toEpochMilli(),
            alarmIntent(context, reminder.id, next.toLocalDate().toEpochDay()),
        )
    }
    ...
    private suspend fun nextOccurrence(
        ...
        isComplete: suspend (LocalDate) -> Boolean,
    ): ZonedDateTime? {
    ...
    fun clearFired(context: Context, id: String) =
        context.getSharedPreferences(FIRED_PREFS, Context.MODE_PRIVATE).edit().remove(id).apply()

    private fun alarmIntent(context: Context, id: String, day: Long) =
        PendingIntent.getBroadcast(
            context,
            requestCode(id),
            Intent(context, ReminderReceiver::class.java)
                .setAction(ACTION_FIRE)
                .setData(Uri.parse("babylog://reminder/$id"))
    ...
    internal suspend fun fire(context: Context, id: String, day: Long) {
        ...
        if (!reminder.occursOn(date) || dao.reminderCompletion(id, day) != null) {
    ...
    private fun show(context: Context, reminder: BabyReminder, day: Long): Boolean {
        ...
        val done =
            PendingIntent.getBroadcast(
                context,
                requestCode(reminder.id) + 1,
                Intent(context, ReminderReceiver::class.java)
                    .setAction(ACTION_DONE)
                    .setData(Uri.parse("babylog://reminder/${reminder.id}/done/$day"))
        ...
        (context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager)
            .createNotificationChannel(
                NotificationChannel(CHANNEL, "Напоминания", NotificationManager.IMPORTANCE_DEFAULT)
                    .apply {
                        description = "Тихие напоминания об уходе"
                        setSound(null, null)
    ...
    private fun hasFired(context: Context, id: String, day: Long) =
        context
            .getSharedPreferences(FIRED_PREFS, Context.MODE_PRIVATE)
            .getLong(id, Long.MIN_VALUE) == day

    private fun markFired(context: Context, id: String, day: Long) =
        context
            .getSharedPreferences(FIRED_PREFS, Context.MODE_PRIVATE)
            .edit()
            .putLong(id, day)
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        CoroutineScope(SupervisorJob() + Dispatchers.IO).launch {
            try {
                ...
                val day =
                    intent.getLongExtra(ReminderScheduler.EXTRA_DAY, LocalDate.now().toEpochDay())
                val app = context.applicationContext as BabyLogApp
                if (intent.action == ReminderScheduler.ACTION_DONE)
                    app.reminderRepository.complete(id, day)
                else ReminderScheduler.fire(context, id, day)
            } finally {
                pending.finish()
            }
        }
