...
class FamilySync(private val app:BabyLogApp){
    ...
    fun startRealtime(){if(!available)return;val owner=runBlocking(Dispatchers.IO){dao.membership()}?:return;listener?.remove();val family=FirebaseFirestore.getInstance().collection("households").document(owner.householdId);listener=family.collection("events").addSnapshotListener{snapshot,error->if(error!=null){status.value=FamilySyncStatus(error=error.message);return@addSnapshotListener};snapshot?:return@addSnapshotListener;app.appScope.launch{try{app.repository.applyRemote(snapshot.documents.mapNotNull{it.data});AppSurfaceSync.refresh(app)}catch(error:Throwable){status.value=FamilySyncStatus(error=error.message?:"Ошибка realtime-синхронизации")}}};family.collection("members").addSnapshotListener{snapshot,_->members.value=snapshot?.documents?.map{FamilyMember(it.id,it.getString("displayName").orEmpty())}.orEmpty()}}
