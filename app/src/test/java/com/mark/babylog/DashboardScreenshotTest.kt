...
class DashboardScreenshotTest {
    ...
    @Test fun dashboard_matches_golden(){val day=java.time.LocalDate.now();val now=day.atTime(19,23).atZone(java.time.ZoneId.systemDefault()).toInstant().toEpochMilli();paparazzi.snapshot{BabyTheme{BabyScreen(UiState(events=listOf(BabyEvent(1,EventType.SLEEP,"LEFT",now-42*60000,null),BabyEvent(2,EventType.FEEDING,"RIGHT",now-2*3600000,now-105*60000)),segments=listOf(SleepSegment(1,1,SleepPosition.LEFT,now-42*60000,null)),selectedDay=day),fixedNow=now)}}}
    ...
    @Test fun activeFeeding_showsRedStopInBottomActions(){val day=java.time.LocalDate.now();val now=day.atTime(20,0).atZone(java.time.ZoneId.systemDefault()).toInstant().toEpochMilli();paparazzi.snapshot{BabyTheme{BabyScreen(UiState(events=listOf(BabyEvent(1,EventType.FEEDING,"LEFT",now-73_000)),selectedDay=day),fixedNow=now)}}}
    @Test fun pumpingDialog_defaultsToOppositeOfLastFeeding(){paparazzi.snapshot{BabyTheme{PumpingDialog(FeedingKind.RIGHT,{}){_,_->}}}}
    @Test fun pumping_isShownInHistory(){val day=java.time.LocalDate.now();val now=day.atTime(19,23).atZone(java.time.ZoneId.systemDefault()).toInstant().toEpochMilli();paparazzi.snapshot{BabyTheme{BabyScreen(UiState(events=listOf(BabyEvent(1,EventType.PUMPING,"RIGHT:120",now,now),BabyEvent(2,EventType.FEEDING,"LEFT",now-2*3600000,now-105*60000)),selectedDay=day),fixedNow=now)}}}
