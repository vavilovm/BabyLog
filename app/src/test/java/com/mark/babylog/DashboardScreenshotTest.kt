...
class DashboardScreenshotTest {
    ...
    @Test fun pumpingFilter_showsOnlyPumping(){val day=java.time.LocalDate.now();val now=day.atTime(19,23).atZone(java.time.ZoneId.systemDefault()).toInstant().toEpochMilli();paparazzi.snapshot{BabyTheme{BabyScreen(UiState(events=listOf(BabyEvent(1,EventType.PUMPING,"LEFT:90",now,now),BabyEvent(2,EventType.FEEDING,"RIGHT",now-60_000,now))),fixedNow=now,initialPumpingOnly=true)}}}
    @Test fun bottleStop_asksForVolume(){paparazzi.snapshot{BabyTheme{BottleVolumeDialog({},{},{})}}}
