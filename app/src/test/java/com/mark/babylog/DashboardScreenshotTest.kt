...
class DashboardScreenshotTest {
    ...
    @Test fun bottleSummary_showsLastAndNextBreast(){val now=System.currentTimeMillis();paparazzi.snapshot{BabyTheme{BabyScreen(UiState(events=listOf(BabyEvent(1,EventType.FEEDING,"BOTTLE",now-10*60_000,now-5*60_000),BabyEvent(2,EventType.FEEDING,"LEFT",now-3*60*60_000,now-2*60*60_000))),fixedNow=now)}}}
    @Test fun editTime_hasMinuteShortcuts(){val now=System.currentTimeMillis();paparazzi.snapshot{BabyTheme{EditDialog(BabyEvent(1,EventType.FEEDING,"LEFT",now-30*60_000,now-5*60_000),{},{},{})}}}
}
